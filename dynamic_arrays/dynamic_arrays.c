...
int array_init(array* arr1)
{
    arr1->data = malloc(2*sizeof(array_data));
    if (!arr1->data)
        return 0;
    arr1->capacity = 2;
    arr1->size = 0;
...
int resize_if_needed(array* arr1)
{
    if (arr1->size >= arr1->capacity)
    {
        int new_capacity = arr1->capacity*2;
        void* temp = realloc(arr1->data, new_capacity*sizeof(array_data));
        if (!temp)
            return 0;
        arr1->data = temp;
